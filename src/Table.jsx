...
const CrudTable = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    atcrelatedLabel: "",
    enabled: true,
    guid: "", // Add guid to formData state
  });
  ...
  const handleEdit = async (guid, index) => {
    try {
      const editedItem = { ...data[index], guid }; // Include guid in editedItem
      await Axios.put(`/api/atc/v1.0`, editedItem);
  ...
  const handleDelete = async (guid) => {
    try {
      await Axios.delete(`/api/atc/v1.0/${guid}`);
  ...
  return (
    <div className="container mx-auto">
      <table className="min-w-full table-auto">
        <thead>
          <tr>
            <th className="px-4 py-2">GUID</th>
            ...
            <tr key={item.guid}>
              ...
              <td className="border px-4 py-2">
                <button
                  ...
                  onClick={() => handleEdit(item.guid, index)}
                >
                  Save
                </button>
                <button
