...
const Register = ({ switchToLogin }) => {
  ...
  return (
    <div className="flex justify-center items-center 2xl:mt-[-4rem] lg:h-full">
      {success ? (
        <section className="success-section bg-white-bg dark:bg-black-bg dark:text-white-text shadow-md rounded-xl p-8 max-w-sm w-full">
          <h1>Success!</h1>
          <p>
            <a href="auth">Sign In</a>
          </p>
          <div>
            Redirecting to <Link to="/profile">your profile page</Link> in {countdown} seconds.
          </div>
        </section>
      ) : (
        <section className="register-section text-left bg-white-bg dark:bg-black-bg dark:text-white-text shadow-md rounded-xl p-8 max-w-sm w-full">
          <p ref={errRef} className={errMsg ? 'offscreen' : 'offscreen'} aria-live="assertive">
            {errMsg}
          </p>
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor="username">
              Username:
              <FontAwesomeIcon icon={faCheck} className={validName ? 'valid ml-2' : 'hide'} />
              <FontAwesomeIcon
                icon={faTimes}
            ...
            >
              {userFocus && UserName && !validName && (
                <>
                  <FontAwesomeIcon className="mr-2 dark:text-white-bg" icon={faInfoCircle} />
            ...
            <div className="password-container flex flex-col my-2 sm:mt-4">
              <label htmlFor="password">
                ...
                <FontAwesomeIcon
                  icon={faTimes}
                  className={validPassword || !Password ? 'hide' : 'invalid valid ml-2'}
                />
              </label>
            ...
            <div className="password-container flex flex-col my-2 sm:mt-4">
              <label htmlFor="confirm_Password">
                ...
                <FontAwesomeIcon
                  icon={faCheck}
                  className={validMatch && matchPassword ? 'valid ml-2' : 'hide'}
                />
                <FontAwesomeIcon
                  icon={faTimes}
                  className={validMatch || !matchPassword ? 'hide' : 'invalid ml-2'}
                />
              </label>
              <div className="relative">
                <input
            ...
            <div className="flex flex-col pt-8">
              ...
              <div className="flex w-full justify-between items-center mt-2">
                ...
                <button
                  className="w-28 cursor-pointer bg-green-500 text-white rounded-md py-2 hover:bg-green-600 transition duration-300"
