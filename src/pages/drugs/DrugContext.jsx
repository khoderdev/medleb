...
//       // If it's the last step, submit form data
//       if (currentStep === steps.length - 1) {
//         const response = await Axios.post('http://localhost:3000/drugs/', formData);
...
//       const atcResponse = await Axios.get(`/api/atc/v1.0`);
//       const atcItems = Array.isArray(atcResponse.data) ? atcResponse.data : [];
//       const atcCodeResponse = await Axios.get(`/api/atccodes/v1.0/codes/${ATCGuid}`);
...
//   return <DrugContext.Provider value={contextValues}>{children}</DrugContext.Provider>;
// };
