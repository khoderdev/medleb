...
const SearchComponent = () => {
  ...
  // const singleDrugRef = useRef(null); // Ref for SingleDrug component

  // // const handleCardClick = (index, guid) => {
  ...
  // const handleCardClick = (guid) => {
  //   setClickedCardIndex(guid);
  //   // Update the ref with the guid associated with the clicked card
  //   singleDrugRef.current = guid;
  // };

  // Function to handle accessing data from SingleDrug component
  // const handleAccessData = () => {
  //   if (singleDrugRef.current !== null) {
  //     console.log(singleDrugRef.current);
  //   }
  // };
  ...
  const handleAlphabetFilterClick = async (letter) => {
    try {
      const response = await axios.get(
        `http://localhost:3000/drugs/search/brand/${letter}`
      );
